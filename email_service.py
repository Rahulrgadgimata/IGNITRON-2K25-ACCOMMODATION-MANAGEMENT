...
def init_email(app):
    ...
    app.config['MAIL_USERNAME'] = os.environ.get('MAIL_USERNAME') or app.config.get('MAIL_USERNAME', '')
    app.config['MAIL_PASSWORD'] = os.environ.get('MAIL_PASSWORD') or app.config.get('MAIL_PASSWORD', '')
    app.config['MAIL_DEFAULT_SENDER'] = os.environ.get('MAIL_USERNAME') or app.config.get('MAIL_USERNAME', '')
...
def send_booking_approval_email(user_email, user_name, room_no, room_description=None):
    ...
    try:
        ...
        if not sender_email or not sender_password:
            ...
            print("   Configure MAIL_USERNAME and MAIL_PASSWORD in config.py or environment variables.")
            return False
        ...
        print(f"   2. Select 'Mail' and generate App Password")
        print(f"   3. Copy the 16-character password (looks like: abcd efgh ijkl mnop)")
        print(f"   4. Update config.py: MAIL_PASSWORD = 'your-app-password-here'")
        print(f"   5. Remove spaces from the password when pasting")
        return False
        ...
        print(f"\n💡 Tip: Check your internet connection and Gmail account settings.")
        return False
